...
const locations = [
  { id: 1, name: "املاک ارنجک", lat: 36.2927, lng: 50.0417, image: "/images/mapimage/p1.jpg" },
  { id: 2, name: "لوازم اداری تاکستان", lat: 36.0708, lng: 49.6845, image: "/images/mapimage/p2.jpg" },
  { id: 3, name: "مبل فروشی", lat: 36.1387, lng: 49.2101, image: "/images/mapimage/p3.jpg" },
  { id: 4, name: "لباس فروشی", lat: 36.1386, lng: 49.2098, image: "/images/mapimage/p4.jpg" },
];
...
const ImageOverlay = ({ location }) => {
  ...
  const imageIcon = useMemo(
    () =>
      L.divIcon({
        html: `<img src="${location.image}" alt="${location.name}" 
          style="width:300px; height:169px; border-radius:8px; border:2px solid #4CAF50;" />`,
        className: "custom-image-marker",
        iconSize: [100, 80],
        iconAnchor: [145, 263],
      }),
...
export default function MapComponent() {
  ...
  return (
    <div id="sample" className="flex flex-col items-center py-5 mt-10">
      <div className="flex flex-row-reverse gap-2 justify-center w-full">
        <p className="mb-5 text-2xl text-right">روی مکان‌ها بروید تا تصویر نمایش داده شود</p>
      ...
      {L && (
        ...
        >
          ...
          {hoveredLocation && <ImageOverlay location={hoveredLocation} />}
        </MapContainer>
      )}
    </div>
  );
